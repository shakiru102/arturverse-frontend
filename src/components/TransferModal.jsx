...
const TransferModal = ({
  ...
  const initiateTransfer = async () => {
       try {
          ...
          const transfer = await contract.transferFrom(account, to, parseInt(item.id))
          // await tokenId.wait()
          console.log(transfer)
          onClose()
       } catch (error) {
          alert('Can not tranfer certificate')
       }
  }
...
return (
  <div  className='fixed top-0 left-0 w-full flex justify-center items-center h-screen bg-[#2d2d2d45]'>
    <div className='bg-gray-950 relative z-50 flex flex-col w-[500px] p-8 shadow-lg'>
        <input value={to} onChange={e => setTo(e.target.value)}  placeholder='Address' className='mb-3 px-2 bg-transparent border text-white border-gray-700 rounded-lg py-2' />
